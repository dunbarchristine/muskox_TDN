...
mod_therm_sum_4 <- glmmTMB(Muskox ~ scale(Water) + scale(log_esker_camera_distances) + scale(TRI_extracted) + scale(fire_age1) + scale(fire_age4) + scale(fire_age3) + scale(fire_age0) +
...
mod_therm_sum_4.4 <- glmmTMB(Muskox ~ scale(Water) + scale(log_esker_camera_distances) + scale(TRI_extracted) + 
                             offset(log(n_days_effort)) + (1|cluster), family="nbinom2", data = model_variables %>% filter(season == "Summer")) #all variables I think support thermoregulation hypothesis
...
step_trees_shrub_both <- stepAIC(mod_global_sum_trees_2, direction="both", trace = TRUE) 


summary(mod_global_sum_shrub)
AIC(mod_global_sum_shrub)

model.matrix(mod_global_sum_shrub)
...
#adding fire ages 0 and 1 together to create new variables called Fire_0 (ages 0-20 yrs old)
model_variables <- model_variables %>%
  mutate(Fire_0 = fire_age0 + fire_age1)

#adding fire ages 2, 3 and 4 together to create new variables called Fire_1 (21 - > 30, yrs old + unburned)
model_variables <- model_variables %>%
  mutate(Fire_1 = fire_age2 + fire_age3 + fire_age4)
