...
#running global summer model with shrub and not trees. fire_age4 is giving NAs, deleted fire_age4 as it should be used as a reference. removed scale from Fire_1
mod_global_sum_shrub <- glmmTMB(Muskox ~ scale(log_esker_camera_distances) + scale(fire_age3) + scale(fire_age0) + scale(fire_age1) + scale(fire_age4) + scale(Shrub) + scale(Groundcover) + scale(Predators) + scale(Water) + scale(TRI_extracted) + scale(Arctic_DEM_500m_elevation_m) +
                                  offset(log(n_days_effort)) + (1|cluster), family="nbinom2", data = model_variables %>% filter(season == "Summer")) 
...
mod_global_sum_trees_2 <- glmmTMB(Muskox ~ scale(log_esker_camera_distances) + scale(fire_age3) + scale(fire_age0) + scale(fire_age1) + scale(fire_age4) + scale(tree_shrub) + scale(Groundcover) + scale(Predators) + scale(Water) + scale(TRI_extracted) + scale(Arctic_DEM_500m_elevation_m) +
...
#   ) +
#   expand_limits(y = c(-max(abs(comp3$Upper)), max(abs(comp3$Upper)))) +
#   theme(
...
#old stepAIC code
# scope <- list(lower = ~ offset(log(n_days_effort) + (1|cluster), upper = ~scale(log_esker_camera_distances) + scale(fire_age3) + scale(fire_age0) + scale(fire_age1) + scale(fire_age4) + scale(tree_shrub) + scale(Groundcover) + scale(Predators) + scale(Water) + scale(TRI_extracted) + scale(Arctic_DEM_500m_elevation_m)) + offset(log(n_days_effort)) + (1|cluster))
#                                   
# 
# step_trees_shrub_both <- stepAIC(mod_global_sum_trees_2, direction="backward", trace = TRUE, scope = scope)
# 
# stepAIC(mod_global_sum_trees_2, direction="backward", trace = TRUE)
# #stepAIC(mod_global_sum_trees_3, direction="backward", trace = TRUE)

# summary(step_trees_shrub_both)
# step_trees_shrub_both$anova
# step_trees_shrub_both

#mod_global_sum_trees_2$anova

#show(mod_global_sum_trees_2)
