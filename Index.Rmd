...
#scaling the variables. I had the scale function in front of each variable, but we tried this way to see if the stepAIC function would work. It wasnt working before when I had the scale in front of each variable. It still did not run when trying it this way

model_data <- model_variables %>% filter(season == "Summer" )%>% mutate(tree_shrub = Trees - Shrub)
...
#original model with all variables. fire_age2 was removed due to high collinearity with fire_age4
mod_global_sum_trees_2 <- buildglmmTMB(Muskox ~ z.log_esker_camera_distances + z.fire_age3 + z.fire_age0 + scale(fire_age1) + scale(fire_age4) + scale(tree_shrub) + scale(Groundcover) + scale(Predators) + scale(Water) + scale(TRI_extracted) + scale(Arctic_DEM_500m_elevation_m) +
...
mod_global_sum_shrub <- glmmTMB(Muskox ~ scale(Water) + scale(log_esker_camera_distances) + scale(TRI_extracted) + scale(fire_age1) + scale(fire_age4) + scale(fire_age3) + scale(fire_age0) + scale(Arctic_DEM_500m_elevation_m) + scale(Shrub) + scale(Groundcover) + scale(Predators) +
                             offset(log(n_days_effort)) + (1|cluster), family="nbinom2", data = model_variables %>% filter(season == "Summer"))
...
fmListAll <- model.sel(mod_null_all_1=mod_null_all_1, mod_pred_sum_1=mod_pred_sum_1, mod_food_sum_3=mod_food_sum_3, mod_food_sum_3.1=mod_food_sum_3.1, mod_food_sum_3.2=mod_food_sum_3.2, mod_food_sum_3.3=mod_food_sum_3.3, mod_food_sum_3.4=mod_food_sum_3.4, mod_food_sum_3.5=mod_food_sum_3.5, mod_food_sum_3.6=mod_food_sum_3.6, mod_food_sum_3.7=mod_food_sum_3.7, mod_therm_sum_4=mod_therm_sum_4, mod_therm_sum_4.1=mod_therm_sum_4.1,
                     mod_therm_sum_4.2=mod_therm_sum_4.2, mod_therm_sum_4.3=mod_therm_sum_4.3, mod_therm_sum_4.4=mod_therm_sum_4.4,mod_therm_sum_4.5=mod_therm_sum_4.5,mod_global_sum_shrub=mod_global_sum_shrub)
...
The approach that I would like to go with for my thesis is the second hypothesis driven approach I have in this document (lines 177 to 254). The best fitting model from this approach is mod2_global (global model, AIC = 4302.4) and the second is mod2_pred_sum_2 (grizzly bears and gray wolves, AIC = 4304.1). These were the only two models that were within 2 delta. Within the global model, the predator and fire_age1 variables best described muskox occurrence in TDN. This aligns with my hypotheses as I predicted muskox detections would increase with the increase of burned areas that were 11-20 years old (fire_age1). I also hypothesized that as muskox detections would decrease as predator detections increase, and these models support that hypothesis. Another thing to note within mod2_global is the estimate for terrain ruggedness was -0.258, implying that muskox are not selecting areas with higher terrain ruggedness. 
